...
import { ChevronLeft, Search, Info, ChevronRight, Upload, Grid3x3, List, CheckCircle2 } from 'lucide-react';
...
export const FilzaFileManager: React.FC<FilzaFileManagerProps> = ({ onItemClick }) => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col max-w-md mx-auto relative overflow-hidden">
      ...
      {/* Success Message Overlay - Ajustado para não colar no topo */}
      {isActivated && (
      ...
      {/* Header - CORRIGIDO: pt-12 e pb-3 para dar espaço ao relógio do iOS */}
      <div className="ios-header bg-background/95 px-4 pt-12 pb-3 flex items-center justify-between border-b border-border">
        <button
          onClick={handleBack}
        ...
        <button className="text-primary hover:opacity-70 transition-opacity text-sm font-semibold min-w-[50px] text-right">
          ...
          <div className="divide-y divide-border">
            {APPS.map((app, index) => (
              <div
                ...
                className="px-4 py-3 flex items-center gap-3 hover:bg-secondary/50 transition-colors duration-150 cursor-pointer"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wider font-bold">{app.size}</p>
                </div>

                <button
                  onClick={(e) => e.stopPropagation()}
                  className="ml-1 p-2 text-primary hover:opacity-70 flex-shrink-0"
                >
                  <Info size={18} />
                </button>
          ...
          <div className="grid grid-cols-3 gap-4 p-4">
            {APPS.map((app, index) => (
              ...
              >
                ...
                <p className="text-[9px] text-muted-foreground uppercase font-bold">{app.size}</p>
              </div>
            ))}
          </div>
        )}
